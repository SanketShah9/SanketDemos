...
#define HOME_TOP_VIEW_HEIGHT 40
...
#define MAX_SHIFT_PER_SCROLL    20  // used when _shouldHideUIBarsGradually=YES
...
- (void)showUIBarsAnimated:(BOOL)animated completion:(void (^)(BOOL finished))completion
{
    ...
    if (animated) {
        ...
        [UIView animateWithDuration:self.showHideAnimationDuration animations:^{
            ...
            if (self.topView) {
                [weakSelf _layoutUIBarsWithDeltaY:-HOME_TOP_VIEW_HEIGHT];
            }
            else{
            ...
            if ([_delegate respondsToSelector:@selector(barShown:)]) {
               if(!self.scrollView.isDragging) [_delegate barShown:weakSelf];
            }
        ...
        if (self.topView) {
            [self _layoutUIBarsWithDeltaY:-HOME_TOP_VIEW_HEIGHT];
        }
        else{
...
- (void)observeValueForKeyPath:(NSString *)keyPath ofObject:(id)object change:(NSDictionary *)change context:(void *)context
{
    if (context == &__fullScreenScrollContext) {
        ...
        if ([keyPath isEqualToString:@"tintColor"]) {
            ...
            // For showing top animation when comes to top
            if (offsetY<50) {
                //NSLog(@"Offset : %f and delts = %f",offsetY,deltaY);
                if(!self.areUIBarsAnimating)[self showUIBarsAnimated:YES];
            }
            else{
...
-(void)showNavBar:(CGFloat)deltaY{
    if (!self.enabled) return;
    if (!self.layoutingUIBarsEnabled) return;
    ...
    if (isNavigationBarExisting && _shouldHideNavigationBarOnScroll && self.topView==nil) {
        ...
        if (canLayoutUIBars) {
            ...
            //}
            //NSLog(@"Top:%f && Delta:%f",navBar.top,deltaY);
            //            if (navBar.top==1 && deltaY<0) {
...
- (void)_layoutUIBarsWithDeltaY:(CGFloat)deltaY
{
    ...
    if (self.topView && _shouldHideNavigationBarOnScroll) {
        if (canLayoutUIBars) {
            _defaultNavBarTop = 0;
            self.topView.top = MIN(MAX( self.topView.top-deltaY, _defaultNavBarTop- self.topView.height), _defaultNavBarTop);
...
- (void)viewWillAppear:(BOOL)animated
{
    ...
    if (self.enabled) {
        ...
        if(!self.isUseOwnViewWillAppear)
            [self showUIBarsAnimated:NO];
    }
}
